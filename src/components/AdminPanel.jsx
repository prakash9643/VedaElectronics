...
export default function AdminPanel() {
  ...
  return <main className={`cms-shell ${sidebarOpen ? '' : 'cms-shell-collapsed'}`}>
    ...
    <section className="cms-main">
      ...
      {section === 'settings' && <section className="cms-panel cms-status-panel"><span className="cms-eyebrow">Workspace settings</span><h2>Settings</h2><p className="cms-panel-intro">Your admin workspace is connected to the local Node.js backend.</p><div className="cms-health-list"><div><span>API status</span><strong className="health-good">Online</strong></div><div><span>Storage</span><strong className="health-good">JSON database</strong></div><div><span>Current admin session</span><strong>Active</strong></div></div><div className="cms-help-box"><strong>Security note</strong><p>Use a strong ADMIN_KEY in .env before deploying this dashboard publicly. Never commit .env or expose Resend credentials in frontend code.</p></div></section>}
    </section>
  </main>
}
